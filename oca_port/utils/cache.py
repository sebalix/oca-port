...
class UserCache:
    ...
    def _get_ported_commits_path(self):
        ...
        file_name = (
            f"{self.settings.addon}_{self.settings.from_branch.name}_"
            f"to_{self.settings.to_branch.name}.list"
        )
    ...
    def _get_commits_to_port_path(self):
        ...
        file_name = (
            f"{self.settings.addon}_{self.settings.from_branch.name}_"
            f"to_{self.settings.to_branch.name}.json"
        )
        return self.dir_path.joinpath(
