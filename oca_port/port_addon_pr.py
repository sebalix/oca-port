...
class PortAddonPullRequest(Output):
    def __init__(self, app, create_branch=True, push_branch=True):
        ...
        self._results = {"process": "port_commits", "results": None}

    def run(self):
        self._print(
            f"{bc.BOLD}{self.settings.addon}{bc.END} already exists "
            f"on {bc.BOLD}{self.settings.to_branch.name}{bc.END}, "
            "checking PRs to port..."
        )
        branches_diff = BranchesDiff(self.app)
        branches_diff.print_diff(self.settings.verbose)
    ...
    def _port_pull_requests(self, branches_diff):
        ...
        base_ref = branches_diff.settings.to_branch  # e.g. 'origin/14.0'
        ...
        for pr, commits in branches_diff.commits_diff.items():
            current_commit = self.settings.repo.commit(self.settings.to_branch.ref())
            ...
            if pr_branch:
                ...
                if self.settings.repo.commit(pr_branch.ref()) == current_commit:
    ...
    def _port_pull_request_commits(
        ...
        if self.create_branch:
            branch_name = PR_BRANCH_NAME.format(
                from_branch=self.settings.from_branch.name,
                to_branch=self.settings.to_branch.name,
                pr_number=pr.number,
            )
            if branch_name in self.settings.repo.heads:
                ...
                if previous_pr_branch:
                    based_on_previous = self.settings.repo.is_ancestor(
                ...
                if not click.confirm(confirm):
                    return g.Branch(self.settings.repo, branch_name), based_on_previous
                self.settings.repo.delete_head(branch_name, "-f")
            ...
            self.settings.repo.git.checkout(
                "--no-track", "-b", branch_name, base_ref.ref()
            )
        else:
            branch_name = self.settings.to_branch.name
        ...
        for commit in commits:
            ...
            try:
                ...
                if not click.confirm(
                    ...
                    self.settings.repo.git.am("--abort")
                    continue
        return g.Branch(self.settings.repo, branch_name), based_on_previous
    ...
    def _push_branch_to_remote(self, branch):
        ...
        confirm = (
            ...
            f"to remote '{bc.BOLD}{self.settings.fork}{bc.END}'?"
        )
        if click.confirm(confirm):
            branch.repo.git.push(self.settings.fork, branch.name, "--force-with-lease")
            branch.remote = self.settings.fork
            return True
        return False
    ...
    def _prepare_pull_request_data(self, processed_prs, pr_branch):
        if len(processed_prs) > 1:
            title = (
                f"[{self.settings.to_branch.name}][FW] {self.settings.addon}: multiple ports "
                f"from {self.settings.from_branch.name}"
            )
            lines = [f"- #{pr.number}" for pr in processed_prs]
            body = "\n".join(
                [
                    f"Port of the following PRs from {self.settings.from_branch.name} "
                    f"to {self.settings.to_branch.name}:"
                ]
                + lines
            )
        else:
            pr = processed_prs[0]
            title = f"[{self.settings.to_branch.name}][FW] {pr.title}"
            body = (
                f"Port of #{pr.number} from {self.settings.from_branch.name} "
                f"to {self.settings.to_branch.name}."
            )
        return {
            "draft": True,
            "title": title,
            "head": f"{self.settings.user_org}:{pr_branch.name}",
            "base": self.settings.to_branch.name,
            "body": body,
        }
    ...
    def _search_pull_request(self, base_branch, title):
        params = {
            "q": (
                f"is:pr "
                f"repo:{self.settings.upstream_org}/{self.settings.repo_name} "
                f"base:{base_branch} "
    ...
    def _create_pull_request(self, pr_branch, pr_data, processed_prs):
        ...
        if click.confirm(
            ...
            f"against {bc.BOLD}{self.settings.upstream_org}/{self.settings.repo_name}{bc.END}?"
        ):
            response = github.request(
                f"repos/{self.settings.upstream_org}/{self.settings.repo_name}/pulls",
                method="post",
                json=pr_data,
            )
...
class BranchesDiff(Output):
    ...
    def _get_branch_commits(self, branch, path="."):
        ...
        """
        commits = self.settings.repo.iter_commits(branch, paths=path)
        commits_list = []
        commits_by_sha = {}
        for commit in commits:
    ...
    def print_diff(self, verbose=False):
        ...
        if fake_pr:
            ...
            message = (
                ...
                f"{bc.ENDC}' to port from {self.settings.from_branch.ref()} "
                f"to {self.settings.to_branch.ref()}"
            )
        else:
            message = (
                f"{bc.BOLD}{bc.OKBLUE}{i} pull request(s){bc.END} "
                f"related to '{bc.OKBLUE}{self.path}{bc.ENDC}' to port from "
                f"{self.settings.from_branch.ref()} to {self.settings.to_branch.ref()}"
            )
        ...
        for commit in self.from_branch_path_commits:
            ...
            if pr:
                for pr_commit_sha in pr.commits:
                    try:
                        raw_commit = self.settings.repo.commit(pr_commit_sha)
                    except ValueError:
    ...
    def _get_original_pr(self, commit: g.Commit):
        ...
        if not any("github.com" in remote.url for remote in self.settings.repo.remotes):
            return
        raw_data = github.get_original_pr(
            self.settings.upstream_org,
            self.settings.repo_name,
            self.settings.from_branch.name,
            commit.hexsha,
        )
        if raw_data:
            ...
            pr_commits_data = github.request(
                f"repos/{self.settings.upstream_org}/{self.settings.repo_name}"
